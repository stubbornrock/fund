...
def fetch(start,end,thread_name):
    my = MYSQL(thread_name)
    url_set = tuple(urls[start:end])
    sql = "UPDATE fund SET updated=False WHERE url in %s" %str(url_set)
    my.update_data(sql)
...
if __name__ == '__main__':
    ...
    if style == "all":
        ...
        us = patch_sl.get_datas("select url from fund where updated=False;")
        urls = [url[0] for url in us]
        patch_sl.close()
    elif style == "one":
        url = sys.argv[2]
    ...
    for i in range(100):
        ...
        th = threading.Thread(name=thread_name,target=fetch,args=(start,end,thread_name))
        threads.append(th)
